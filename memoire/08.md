...
> - Objectif : préserver la haute disponibilité (réplication transactionnelle, supervision, bascule) lors de toute intervention.
> - Risques évités : désynchronisation, indisponibilité collecte/diffusion, déploiements incompatibles avec la réplication.
> - Réponse du titulaire : analyse d'impact SQL/HD, évolutions compatibles, Go/No‑Go + rollback, tests de bascule documentés.
> - Preuves : Chapitre 8, Annexe A.2 (tests NR), Annexe A.1 (livraison/rollback), Annexe A.4 (audit).
...
### 8.2.6 Tests de bascule (objectif, modalités, preuves)

Un **test de bascule** consiste à vérifier, en conditions contrôlées, la capacité à passer d'un **site actif** à un **site passif** (et retour), ainsi que le bon fonctionnement du **secours automatique** (collectes/échanges) et de la **supervision** associée. L'objectif est de s'assurer que la haute disponibilité reste opérable après des changements (mise à jour, correctif, évolution, reparamétrage) et que les procédures d'exploitation restent valides.

**Quand réaliser un test de bascule**
- lors de l'installation du fonctionnement d'ensemble (prestation HD) ;
- lors des **audits** des instances en configuration HD (CCTP : volet tests HD) ;
- en cas d'évolution à risque (impacts réplication, supervision, services de collecte/échange), avant décision de Go/No‑Go.

**Livrables et traçabilité**
- résultats consignés dans un rapport (audit / tests) et rattachés aux tickets concernés ;
- décisions et actions suivies en points de suivi / COPIL.
